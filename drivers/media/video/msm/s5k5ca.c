...
static uint16_t s5k5ca_model_id = MODEL_LITEON;
...
{0x0F12,  0x0515}, // [11:10] SDA, [9:8] SCA, [7:6] PCLK, [3:2] VSYNC, [1:0] HSYNC
...
{0x002A,0x01F6},
{0x0F12,  0x1770},	//2904	//2BF2 // #REG_TC_IPRM_OpClk4KHz_0
// Set pixel CLK // 60MHz (0x3A98)
{0x0F12,  0x1770}, //0x3A88 // #REG_TC_IPRM_MinOutRate4KHz_0
{0x0F12,  0x1770}, //0x3AA8 // #REG_TC_IPRM_MaxOutRate4KHz_0
{0x0F12,  0x1F40}, // #REG_TC_IPRM_OpClk4KHz_1
{0x0F12,  0x3200}, //0x3A88 //1770 //1194 //157C //1964 //157C //3A88 // #REG_TC_IPRM_MinOutRate4KHz_1
{0x0F12,  0x32c8}, //0x3AA8 //1770 //1770 //1194 //157C //1964 //157C //3AA8 // #REG_TC_IPRM_MaxOutRate4KHz_1
// Update PLL
{0x002A,0x0208},
...
//{0x0F12,0x0140}, //#REG_0TC_PCFG_usWidth//1024   
//{0x0F12,  0x00f0}, //#REG_0TC_PCFG_usHeight //768    026E 
{0x0F12,0x0005}, //#REG_0TC_PCFG_Format            0270
{0x0F12,  0x1770},// //2ee0 //3AA8 //#REG_0TC_PCFG_usMaxOut4KHzRate  0272
{0x0F12,  0x1770},// //2e00 //3A88 //#REG_0TC_PCFG_usMinOut4KHzRate  0274
...
//================================================================================================
{0x002A,0x035C}, 
{0x0F12,0x0001}, //#REG_0TC_CCFG_uCaptureModeJpEG
{0x0F12,0x0800}, //#REG_0TC_CCFG_usWidth 
{0x0F12,0x0600}, //#REG_0TC_CCFG_usHeight
{0x0F12,0x0005}, //#REG_0TC_CCFG_Format//5:YUV9:JPEG 
{0x0F12,  0x32c8}, //#REG_0TC_CCFG_usMaxOut4KHzRate
{0x0F12,  0x3200}, //#REG_0TC_CCFG_usMinOut4KHzRate
...
const static struct s5k5ca_i2c_reg_conf s5k5ca_init_reg_config_liteon_and_byd[] =
{
//================================================================================================= 
// * Name: S5K5CAGX EVT1.0 Initial Setfile 
// * PLL mode: MCLK=24.5MHz / SYSCLK=40MHz / PCLK=60MHz
        ...
        case UPDATE_PERIODIC:
            ...
            if(rt == RES_PREVIEW)
            {
                ...
                if(MODEL_LITEON == s5k5ca_model_id || MODEL_BYD == s5k5ca_model_id)
                {
                    if (machine_is_msm7x25_u8300())
                    {
                         rc = s5k5ca_i2c_write_w_table(s5k5ca_init_reg_config_liteon_and_byd_u8300,
                                                S5K5CA_ARRAY_SIZE(s5k5ca_init_reg_config_liteon_and_byd_u8300));
                    }
                    else
                    {
                ...
                else if(MODEL_SUNNY == s5k5ca_model_id)
                {
                    if (machine_is_msm7x25_u8300())
                    {
                         rc = s5k5ca_i2c_write_w_table(s5k5ca_init_reg_config_sunny_u8300,
                                                S5K5CA_ARRAY_SIZE(s5k5ca_init_reg_config_sunny_u8300));
                    }
                    else
                    {
                ...
                else if(MODEL_FOXCOM == s5k5ca_model_id)
                {
                    if (machine_is_msm7x25_u8300())
                    {
                         rc = s5k5ca_i2c_write_w_table(s5k5ca_init_reg_config_foxcom_u8300,
                                                S5K5CA_ARRAY_SIZE(s5k5ca_init_reg_config_foxcom_u8300));
                    }
                    else
                    {
...
static int s5k5ca_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
	...
	rc = s5k5ca_i2c_write_w(0x0028, 0xD000);
	if (rc < 0)
		goto init_probe_fail;
	rc = s5k5ca_i2c_write_w(0x002A, 0x108E);
	if (rc < 0)
		goto init_probe_fail;
	rc = s5k5ca_i2c_write_w(0x0F12, 0x0033);
	if (rc < 0)
		goto init_probe_fail;    
	rc = s5k5ca_i2c_write_w(0x0F12, 0x0066);
	if (rc < 0)
		goto init_probe_fail;     
    mdelay(2);
    
	rc = s5k5ca_i2c_write_w(0x002C, 0xD000);
	if (rc < 0)
		goto init_probe_fail;
	rc = s5k5ca_i2c_write_w(0x002E, 0x100C);
	if (rc < 0)
		goto init_probe_fail;
	rc = s5k5ca_i2c_read_w(0x0F12, &s5k5ca_model_id);
	if (rc < 0)
	{
        CDBG("s5k5ca_i2c_read_w 0x002e rc=%d", rc);
	}
    CDBG("s5k5ca model = 0x%x\n", s5k5ca_model_id);
    if(s5k5ca_model_id > MODEL_FOXCOM)
    {
        s5k5ca_model_id = MODEL_LITEON;
    }
	...
	if (rc < 0)
		goto init_probe_fail;
	rc = s5k5ca_i2c_write_w(0x0F12, 0x0000);
	if (rc < 0)
		goto init_probe_fail;    
	rc = s5k5ca_i2c_write_w(0x0F12, 0x0000);
	if (rc < 0)
