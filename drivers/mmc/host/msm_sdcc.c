...
msmsdcc_irq(int irq, void *dev_id)
{
	...
	do {
		...
		if ((host->pdev_id == SDCC_WIFI_SLOT)&& !wifi_chip_is_bcm ){
			if (sdio_int_oper) {
					if (readl(host->base + MMCIMASK0) & MCI_SDIOINTOPER) {
						mmc_signal_sdio_irq(host->mmc);
					}
					writel(MCI_SDIOINTOPER, host->base + MMCICLEAR);
			}
		}
#endif
		if (data) {
...
{
	struct mpp *mpp_wlan_chip_down = 0, *mpp_wlan_18v = 0;
	int ret = 0;
   	...
   	if (enable)
	{
		...
		mpp_wlan_18v = mpp_get(NULL, WLAN_1_2V_MPP);
		if (!mpp_wlan_18v) 
		{
			printk(KERN_ERR "%s: WL_REG_ON mpp_get failed to up\n", __func__);
			return -EIO;
		}

		/* pull high */
		ret = mpp_config_digital_out(mpp_wlan_18v,
		...
		// WLAN chip to reset
		mpp_wlan_chip_down = mpp_get(NULL, WLAN_CHIP_PWD);
		if (!mpp_wlan_chip_down) 
		{
			printk(KERN_ERR "%s: EXT_POR_L mpp_get failed\n", __func__);
			return -EIO;
		}
		/* pull up */
		...
		if (!mpp_wlan_18v) 
		{
			printk(KERN_ERR "%s: WL_REG_ON level mpp_get failed to down\n", 
					__func__);
			return -EIO;
		}
		ret = mpp_config_digital_out(mpp_wlan_18v,
		...
		if (!mpp_wlan_chip_down) 
		{
			printk(KERN_ERR "%s: EXT_POR_L failed\n", __func__);
			return -EIO;
		}
		ret = mpp_config_digital_out(mpp_wlan_chip_down,
...
int is_wlan_slot(int dev_id)
{
	return (WLAN_SDCC_ID == dev_id);
}
...
{
    struct mpp *mpp_wlan_chip_down = 0, *mpp_wlan_12v = 0;
    int ret = 0;
    struct vreg *vreg = NULL;
    ...
    if (enable)
    {
        ...
        mpp_wlan_12v = mpp_get(NULL, WLAN_1_2V_MPP);
        if (!mpp_wlan_12v) 
        {
            printk(KERN_ERR "%s: 1.2V voltage level mpp_get failed to up\n", __func__);
            return -EIO;
        }
        ...
        if (!mpp_wlan_chip_down) 
        {
            printk(KERN_ERR "%s: wlan chip down mpp_get failed\n", __func__);
            return -EIO;
        }
        /* pull up */
        ret = mpp_config_digital_out(mpp_wlan_chip_down,
    ...
    else 
    {
        ...
        if (!mpp_wlan_12v) 
        {
            printk(KERN_ERR "%s: 1.2V  voltage level mpp_get failed to down\n", 
                   __func__);
            return -EIO;
        }
        ret = mpp_config_digital_out(mpp_wlan_12v,
