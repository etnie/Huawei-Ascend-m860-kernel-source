...
static int crashdump_debug = 0; 
static struct class *crashdump_class = NULL; 
static char *crashdump_name = "crashdump"; 
static unsigned long crashdump_base = 0xFFFC000;

static unsigned long crashdump_size = 0x20000; 
...
static int crashdump_open(struct inode *pnode, struct file *filep)
{
    ...
    crashdump_dev->dump_base = ioremap(crashdump_base, crashdump_size); 
...
fail_create_class:
 ...
 void __exit mem_crashdump_exit(void)
{
    crashdump_printf("%s\n", __FUNCTION__); 
...
module_init(mem_crashdump_init); 
module_exit(mem_crashdump_exit); 
