...
static int cypress_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (i == 3) 
	{	
		pr_err("%s:check %d times, but dont find sypress_ts device\n",__FUNCTION__, i);	
	...
	{
		memset(name,0,30);			
		sprintf(name, "cypress-ts-%s.Ver%x%x",((cypid==0x1386) ? "innolux":"byd"),verh,verl);		
    ...
    if(ts->use_touch_key)
    {
		...
		ts->key_input->name = "cypress-touch-keypad";
