...
#define Y_MAX   1024
#define P_MAX   256
/* minish TOUCH_KEY_NULL_REGION,and increase touch key region*/
#define TOUCH_KEY_NULL_REGION  8
...
static int virt_key_calibration_y[EXTRA_MAX_TOUCH_KEY]= {900,900,900};
enum
    {
        HOME_KEY = 0,
        MENU_KEY = 1,
...
{
    /* modify touch region  home_key region menu_key region  back_key region */
    if (((pos_x < (X_MAX/3)) && (pos_y > (virt_key_calibration_y[HOME_KEY] + TOUCH_KEY_NULL_REGION/2)))
        || ((pos_x >= (X_MAX/3)) && (pos_x < (X_MAX*2/3)) && (pos_y > (virt_key_calibration_y[MENU_KEY] + TOUCH_KEY_NULL_REGION/2)))
        || ((pos_x >= (X_MAX*2/3)) && (pos_y > (virt_key_calibration_y[BACK_KEY] + TOUCH_KEY_NULL_REGION/2))))
    {
        return TRUE;
    }
...
static void ts_timer(unsigned long arg)
{
    struct ts *ts = (struct ts *)arg;
     ...
     /* write this register means to open the pen down interrupt 
     * move this action back to ts_interrupt function */
...
static irqreturn_t ts_interrupt(int irq, void *dev_id)
{
    ...
    if (!(readl(TSSC_REG(CTL)) & TSSC_CTL_DATA))
     ...
     /* here to write the register to open up the pen down interrupt
     * moved back from ts_timer function to open up the interrupt immediately after get the ADC result */
...
/*detect a declared line,because it is declared ahead*/
	...
	unsigned long count, void *data)
{
...
#ifdef CONFIG_HUAWEI_TOUCHSCREEN_EXTRA_KEY
    /* virt_key_calibration_y become 3 sections*/
    ...
    for(i=0; i<EXTRA_MAX_TOUCH_KEY; i++)
    {
        TSSC("%s:virt_key_calibration_y[%d] = %d\n!", __FUNCTION__, i, virt_key_calibration_y[i]);
        if(virt_key_calibration_y[i] > (Y_MAX-TOUCH_KEY_NULL_REGION))
        {
            virt_key_calibration_y[i] = (Y_MAX-TOUCH_KEY_NULL_REGION);
            printk(KERN_ERR "error key calibraion oversacel = %d\n",
                     virt_key_calibration_y[i]);
        }
        else if(virt_key_calibration_y[i] < (Y_MAX-TOUCH_KEY_MAX_REGION))
        {
            printk(KERN_ERR "key calibraion value over TOUCH_KEY_MAX_REGION= %d\n",
                     virt_key_calibration_y[i]);
            virt_key_calibration_y[i] = (Y_MAX-TOUCH_KEY_MAX_REGION);
        }
        touch_extra_key_region.extra_key[i].center_y = (Y_MAX + (virt_key_calibration_y[i] + TOUCH_KEY_NULL_REGION)) >> 1;
