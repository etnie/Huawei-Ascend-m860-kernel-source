...
void usb_function_enable(const char *function, int enable)
{
    ...
    else
    {
    	for (i = 0; i < ui->num_funcs; i++) {
            ...
            /* If the ADB enabled, the ADB and the later ports are all enabled
               If the ADB disabled, the ADB and the later ports are all disabled
            */
    		if (!strcmp(function, fi->func->name))
   			{
				if (fi->enabled)
					functions_mask |= (1 << i);
				else
    	...
    	ui->composition->functions = functions_mask;
    	pid = ui->composition->product_id;
    }
