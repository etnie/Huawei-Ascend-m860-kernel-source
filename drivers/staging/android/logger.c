...
#include "logger.h"
#ifdef CONFIG_HUAWEI_NV_LOG_CONTROL
			 ...
			 unsigned long nr_segs, loff_t ppos)
{
    ...
    // ensure userspace log is not too large, if large than 4000 byte, ignore it 		
    if (total_logbytes > 4000)
    {
        printk(KERN_WARNING"%s, pid=%d, total_logbytes=%d\n", __func__, current->pid, total_logbytes);
        return 0;
    }
...
#ifdef CONFIG_HUAWEI_NV_LOG_CONTROL
#define LOG_CTL_INFO_ITEM	   60008

int  log_setting(int is_read, unsigned *log_on_off, unsigned *log_level)
{
    /*4bytes, but the only [0], [1] is usefull*/
    char nv_info[4] = {0};
    unsigned nv_log_ctl_info = LOG_CTL_INFO_ITEM;
    int  rval = -1;

    if(NULL == log_on_off || NULL == log_level)
		goto OUT_ERR;


    if(is_read)
    {
        rval = msm_proc_comm(PCOM_NV_READ, &nv_log_ctl_info, (unsigned*)nv_info) ; 

		if(0 == rval)
		{
		    *log_on_off = nv_info[0];
		    *log_level = nv_info[1];
		    goto OUT_SUCC;
		}
		else
    ...
    {
        nv_info[0] = *log_on_off;
		nv_info[1] = *log_level;
		rval = msm_proc_comm(PCOM_NV_WRITE, &nv_log_ctl_info, (unsigned *)nv_info) ; 
...
OUT_ERR:
    printk(KERN_WARNING"log nv setting  Failed...\n");
    return 0;
	
OUT_SUCC:
    printk(KERN_INFO"log nv setting Success...\n");
    return 1;
}
#endif
			...
			const size_t count)
{
	...
	if (sscanf(buf, "%d", &enabled) != 1)
		return -EINVAL;
	#ifdef CONFIG_HUAWEI_NV_LOG_CONTROL
	rval = log_setting(0, (unsigned *)&enabled, (unsigned *)&log->priority);
	if(1 == rval)
	{
		printk(KERN_INFO "set global log enable value = %d\n", enabled);
	}
	else
	{
		printk(KERN_WARNING "set global enable failed..\n");
	}
	#endif
			...
			const size_t count)
{
	...
	#ifdef CONFIG_HUAWEI_NV_LOG_CONTROL
	rval = log_setting(0, (unsigned *)&log->enabled, (unsigned*)&priority);
	if(1 == rval)
	{
		printk(KERN_INFO "set global log priority value = %d\n", priority);
	}
	else
	{
		printk(KERN_WARNING "set global priority failed..\n");
	}
	#endif
	return strnlen(buf, count);
}
...
static int __init init_log(struct logger_log * const log)
{
	int ret;
	#ifdef CONFIG_HUAWEI_NV_LOG_CONTROL
	unsigned nv_priority = 0;
	unsigned nv_enabled = 0;
	#endif
	...
	#ifdef CONFIG_HUAWEI_NV_LOG_CONTROL
	/*read log's nv setting*/
	ret = log_setting(1, &nv_enabled, &nv_priority);
	if(1== ret)
	{
	    /*change the default value*/
		atomic_set(&log->priority, nv_priority);
		atomic_set(&log->enabled, nv_enabled);
	}
	#endif
