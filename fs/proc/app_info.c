				 ...
				 int count, int *eof, void *data)
{
	...
	switch(machine_arch_type)
	{
		...
		case MACH_TYPE_MSM7X25_M860:
        	...
        	strcpy(hw_version_sub_ver, "HC1M860M");
			break;
    ...
    else if(MACH_TYPE_MSM7X25_C8150 == machine_arch_type)
    {
    ...
    else if(MACH_TYPE_MSM7X25_U8150 == machine_arch_type)
    {
        ...
        {
            sprintf(sub_ver, "_EU.Ver%c", 'A');
        }
        /*if sub_board_id is equal to 1(VerB), the product is U8150_JP.VerA */
        else if(HW_VER_SUB_VB == get_hw_sub_board_id())
        {
            sprintf(sub_ver, "_JP.Ver%c", 'A');
        }
	...
	"ts_id:\n%d\n"
	"hw_version:\n%s\n",
	appsboot_version, ker_ver, str_flash_nand_id, s_board_id, lcd_name, camera_id, ts_id, hw_version_sub_ver);
